...
<aside class="callout">
<p><strong>记忆锚点</strong>（来自 Anthropic Barry Zhang）：Workflow 是你拥有管道（you own the plumbing），Agent 是模型拥有管道。其余所有 tradeoff 都派生自这一个结构性选择。</p>
</aside>
...
**Workflow 版**：代码写死三步——调搜索 API 拿前 5 条 → 喂给模型总结 → 套模板输出。永远这三步，无论中间结果好不好。

**Agent 版**：它先搜，发现某家信息不全，自己决定换关键词再搜一次；发现有套餐差异，又去抓官网详情页；最后才写。搜几次、搜什么，是它**临场决定**的。
...
能画出来就别上 Agent——固定 Workflow 在准确率、可控性、成本上都更优。

**2. 任务价值是否高到值得 token 开销？**

Agent 要探索，探索烧钱。粗算 10 美分 / 任务 ≈ 3–5 万 token——这是 Workflow 的地盘。

**3. 错误成本有多高？**

错误高危且难检测时，自主性就变成负债。read-only 权限和 human-in-the-loop 是真实的缓解手段，但也会限制能 scale 的上限。
